...
namespace buddy_hw_interface
{

class BuddySystemPositionOnly : public SystemInterface
{
public:
  CallbackReturn on_init(const HardwareInfo & info) override
  {
    ...
    if (!portHandler_->openPort()) {
      RCLCPP_ERROR(rclcpp::get_logger("BuddySystemPositionOnly"), "Failed to open USB port!");
      return CallbackReturn::ERROR;
    } else {
      RCLCPP_INFO(rclcpp::get_logger("BuddySystemPositionOnly"), "USB port opened successfully!");
    }

    if (!portHandler_->setBaudRate(dxl_baud_rate_)) {
      RCLCPP_ERROR(rclcpp::get_logger("BuddySystemPositionOnly"), "Failed to set baud rate!");
      return CallbackReturn::ERROR;
    } else {
      RCLCPP_INFO(rclcpp::get_logger("BuddySystemPositionOnly"), "Baudrate set successfully!");
    }
  ...
  return_type read(const rclcpp::Time &, const rclcpp::Duration &) override
  {
    for (size_t i = 0; i < joint_names_.size(); ++i)
    {
      ...
      if (dxl_comm_result != COMM_SUCCESS) {
        RCLCPP_INFO(rclcpp::get_logger("BuddySystemPositionOnly"), "Failed to read position for joint %s", joint_names_[i].c_str());
      } else if (dxl_error != 0) {
  ...
  return_type write(const rclcpp::Time &, const rclcpp::Duration &) override {
    for (size_t i = 0; i < joint_names_.size(); ++i) 
    {
      ...
      if (dxl_comm_result != COMM_SUCCESS) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("BuddySystemPositionOnly"),"Error del Dynamixel (ID %d): %s",id, packetHandler_->getRxPacketError(dxl_error));
      }
    }
    return return_type::OK;
  }
...
PLUGINLIB_EXPORT_CLASS(buddy_hw_interface::BuddySystemPositionOnly, hardware_interface::SystemInterface)
